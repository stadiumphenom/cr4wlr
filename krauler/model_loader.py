...
import argparse
from krauler.config import MODEL_REPO_BRANCH
...
def get_home_folder():
    home_folder = os.path.join(
        os.getenv("CRAWL4_AI_BASE_DIRECTORY", Path.home()), ".krauler"
    )
...
def load_spacy_model():
    ...
    if not (model_folder.exists() and any(model_folder.iterdir())):
        repo_url = "https://github.com/unclecode/krauler.git"
        branch = MODEL_REPO_BRANCH
        repo_folder = Path(home_folder) / "krauler"
