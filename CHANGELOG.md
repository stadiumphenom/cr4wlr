...
### Removed
- Obsolete modules under `krauler/browser/*` superseded by the new pooled browser layer  
...
#### Upgrade notes
1. Update any direct imports from `krauler/browser/*` to the new pooled browser modules  
...
#### Added
- **Windows Event Loop Configuration**: Introduced a utility function `configure_windows_event_loop` to resolve `NotImplementedError` for asyncio subprocesses on Windows. ([#utils.py](krauler/utils.py), [#tutorials/async-webcrawler-basics.md](docs/md_v3/tutorials/async-webcrawler-basics.md))
- **`page_need_scroll` Method**: Added a method to determine if a page requires scrolling before taking actions in `AsyncPlaywrightCrawlerStrategy`. ([#async_crawler_strategy.py](krauler/async_crawler_strategy.py))

#### Changed
- **Version Bump**: Updated the version from `0.4.246` to `0.4.247`. ([#__version__.py](krauler/__version__.py))
- **Improved Scrolling Logic**: Enhanced scrolling methods in `AsyncPlaywrightCrawlerStrategy` by adding a `scroll_delay` parameter for better control. ([#async_crawler_strategy.py](krauler/async_crawler_strategy.py))
...
#### Removed
- **Legacy Markdown Generation Code**: Removed outdated and unused code for markdown generation in `content_scraping_strategy.py`. ([#content_scraping_strategy.py](krauler/content_scraping_strategy.py))
...
- **Page Closing to Prevent Memory Leaks**:
  ...
  - **File**: [`async_crawler_strategy.py`](krauler/async_crawler_strategy.py)
  - **Code**:
    ```python
    finally:
        ...
        if not config.session_id:
            await page.close()
    ```
- **Multiple Element Selection**: Modified `_get_elements` in `JsonCssExtractionStrategy` to return all matching elements instead of just the first one, ensuring comprehensive extraction. ([#extraction_strategy.py](krauler/extraction_strategy.py))
- **Error Handling in Scrolling**: Added robust error handling to ensure scrolling proceeds safely even if a configuration is missing. ([#async_crawler_strategy.py](krauler/async_crawler_strategy.py))
...
**Affected Files:**
- `krauler/content_filter_strategy.py`: Enhancement of content filtering capabilities.
```diff
...
- Bumped version number in `krauler/__version__.py` to 0.3.746.
...
### Development Updates
- Developed `post_install` method in `krauler/install.py` to streamline post-installation setup tasks.
- Refined migration processes in `krauler/migrations.py` with enhanced logging for better error visibility.
...
### krauler/__version__.py
Version update.
Bumped version to 0.3.743.

### krauler/async_crawler_strategy.py
...
### krauler/markdown_generation_strategy.py
...
- Users can now specify download folders using the `downloads_path` parameter in the `AsyncWebCrawler` constructor or the `arun` method. If not specified, downloads are saved to a "downloads" folder within the `.krauler` directory.
...
async def browser_management_demo():
    user_data_dir = os.path.join(Path.home(), ".krauler", "user-data-dir")
...
- Removed deprecated: `krauler/content_cleaning_strategy.py`.
...
- Moved version file to `krauler/__version__.py`.
- Added `krauler/cache_context.py`.
- Added `krauler/version_manager.py`.
- Added `krauler/migrations.py`.
- Added `krauler-migrate` entry point.
...
2. **Content Filtering Improvements** (Nov 14, 2024)
   ...
   - Added new file: `krauler/content_filter_strategy.py`
   - Removed deprecated: `krauler/content_cleaning_strategy.py`
...
3. **Local File and Raw HTML Support** (Nov 13, 2024)
   ...
   - Enhanced `krauler/async_webcrawler.py` with significant performance improvements
...
4. Using the Doctor:
   - Run doctor command for system diagnostics: `krauler doctor`
...
- Custom base directory support for cache storage:
  ...
  - Allows specifying alternative locations for `.krauler` folder
...
### Developer Notes
- The customized html2text library is now located within the krauler package
...
These updates aim to provide more flexibility in text processing, improve performance, and enhance the overall capabilities of the krauler library. The new chunking strategies, in particular, offer more options for handling large texts in various scenarios.
...
These updates significantly enhance the flexibility, accuracy, and robustness of krauler, providing users with more control and options for their web crawling and content extraction tasks.
