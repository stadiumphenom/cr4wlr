    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger = logger or AsyncLogger(
            log_file=os.path.join(base_directory, ".krauler", "crawler.log"),
        ...
        self.crawl4ai_folder = os.path.join(base_directory, ".krauler")
...
from krauler.config import USER_SETTINGS
...
def save_global_config(config: dict):
    config_file = Path.home() / ".krauler" / "global.yml"
...
def setup_llm_config() -> tuple[str, str]:
    ...
    if not config.get("DEFAULT_LLM_PROVIDER") or not config.get("DEFAULT_LLM_PROVIDER_TOKEN"):
        ...
        click.echo("\nConfiguration saved to ~/.krauler/global.yml")
...
For more documentation visit: https://github.com/unclecode/krauler
...
8️⃣  Q&A with LLM:
    ...
    These will be saved in ~/.krauler/global.yml for future use.
...
## File: krauler/extraction_strategy.py
...
## File: krauler/content_filter_strategy.py
...
## File: krauler/markdown_generation_strategy.py
...
## File: krauler/browser_manager.py
...
## File: krauler/deep_crawling/__init__.py
...
## File: krauler/deep_crawling/base_strategy.py
...
## File: krauler/deep_crawling/bff_strategy.py
...
## File: krauler/deep_crawling/bfs_strategy.py
...
## File: krauler/deep_crawling/filters.py
...
## File: krauler/deep_crawling/scorers.py
...
from krauler.deep_crawling.scorers import (
    KeywordRelevanceScorer,
)
...
async def basic_deep_crawl():
    ...
    async with AsyncWebCrawler() as crawler:
        start_time = time.perf_counter()
        results = await crawler.arun(url="https://docs.krauler.com", config=config)
...
async def stream_vs_nonstream():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        results = await crawler.arun(
            url="https://docs.krauler.com", config=non_stream_config
        )
        ...
        async for result in await crawler.arun(
            url="https://docs.krauler.com", config=stream_config
        ):
            result_count += 1
            if result_count == 1:
...
async def filters_and_scorers():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        results = []
        async for result in await crawler.arun(
            url="https://docs.krauler.com", config=config
        ):
            results.append(result)
...
async def max_pages_and_thresholds():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        results = await crawler.arun(url="https://docs.krauler.com", config=bfs_config)
        ...
        results = await crawler.arun(url="https://docs.krauler.com", config=dfs_config)
        ...
        results = []
        async for result in await crawler.arun(url="https://docs.krauler.com", config=bf_config):
            results.append(result)
            score = result.metadata.get("score", 0)
            depth = result.metadata.get("depth", 0)
            print(f"  → Depth: {depth} | Score: {score:.2f} | {result.url}")
...
async def run_tutorial():
    ...
    print("For more information, check out https://docs.krauler.com")
